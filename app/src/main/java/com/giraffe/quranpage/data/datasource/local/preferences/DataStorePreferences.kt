package com.giraffe.quranpage.data.datasource.local.preferences
...
class DataStorePreferences(private val context: Context) {
    ...
    suspend fun removeInt(key: String) {
        context.dataStore.edit { it.remove(intPreferencesKey(key)) }
    }
    ...
    suspend fun readBoolean(key: String) = context.dataStore.data.map { preferences ->
        preferences[booleanPreferencesKey(key)] ?: "not found in preferences !!"
    }.first()
