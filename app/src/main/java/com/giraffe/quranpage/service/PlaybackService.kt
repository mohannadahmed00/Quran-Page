...
import com.giraffe.quranpage.receiver.MediaPlayerReceiver
import com.giraffe.quranpage.utils.AudioPlayerManager
import com.giraffe.quranpage.utils.Constants.Actions.NEXT
import com.giraffe.quranpage.utils.Constants.Actions.PAUSE
import com.giraffe.quranpage.utils.Constants.Actions.PLAY
import com.giraffe.quranpage.utils.Constants.Actions.PREVIOUS
import com.giraffe.quranpage.utils.Constants.Actions.RELEASE
import com.giraffe.quranpage.utils.Constants.Actions.STOP
...
class PlaybackService : Service() {
    ...
    var trackAudio: (VerseModel?) -> Unit = {}



    override fun onCreate() {
        super.onCreate()
        Log.d("messi", "onCreate: ")
        initNotificationChannel()
        initMediaSession()
        CoroutineScope(Dispatchers.IO).launch{
            audioPlayer.value.isCompleted.collect{
                Log.d("bale", "isCompleted: $it")
                if (it) startSession()
            }
        }
    ...
    fun setTracker(trackAudio: (VerseModel?) -> Unit){
    ...
    private fun next(){
        seekTo(audioPlayer.value.currentVerse.value?.verseNumber?.plus(1)?:1)
    }

    private fun previous(){
        seekTo(audioPlayer.value.currentVerse.value?.verseNumber?.minus(1)?:0)
    }
